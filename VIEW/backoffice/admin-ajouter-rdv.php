...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($id_citoyen) && !empty($id_service) && !empty($date_heure)){
        $result = $rdvController->adminAjouterRendezVous($id_citoyen, $id_service, $date_heure, $motif);
        if($result){ $success = "Rendez-vous ajouté avec succès ! ID: #".$result; }
...
<div class="container">
    <div class="card reveal">
        <div class="card-header"><h2 class="card-title">Ajouter un rendez-vous</h2></div>
            ...
            <div class="form-group">
                ...
                <select name="id_citoyen" class="form-control" required>
                    ...
                    <?php foreach($citoyens as $c): ?>
                        <option value="<?= $c['id'] ?>"><?= $c['nom'].' '.$c['prenom'].' ('.$c['email'].')' ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
