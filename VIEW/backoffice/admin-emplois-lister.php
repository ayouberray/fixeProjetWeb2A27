...
theme_render_start([
    'title' => theme_t('Gestion des emplois', 'إدارة الجداول'),
    'page_title' => theme_t('Gestion des emplois', 'إدارة الجداول'),
    'page_subtitle' => theme_t('Pilotez les affectations des agents avec un affichage moderne et lisible.', 'قم بادارة تعيينات الاعوان عبر عرض حديث وسهل القراءة.'),
    'nav_context' => 'emplois',
]);
?>
<div class="table-panel">
    <div class="panel-toolbar">
        <div>
            ...
            <h2><?= htmlspecialchars(theme_t('Liste des emplois', 'قائمة الجداول')) ?></h2>
        </div>
        ...
        <div class="table-wrap">
            <table>
                <thead>
                    <tr>
                        <th>ID</th>
                        <th><?= htmlspecialchars(theme_t('Agent', 'العون')) ?></th>
                        <th><?= htmlspecialchars(theme_t('Service', 'الخدمة')) ?></th>
                        <th><?= htmlspecialchars(theme_t('Shift', 'المناوبة')) ?></th>
                        <th><?= htmlspecialchars(theme_t('Date', 'التاريخ')) ?></th>
                        <th><?= htmlspecialchars(theme_t('Statut', 'الحالة')) ?></th>
                    ...
                    <?php foreach ($emplois as $emploi): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="muted">
                                    <?= htmlspecialchars(theme_t('Shift lie', 'المناوبة المرتبطة')) ?>:
                                    #<?= (int) ($emploi['id_shift'] ?? 0) ?>
                                </span>
                            </td>
                            <td><?= htmlspecialchars(date('d/m/Y', strtotime($emploi['date_travail']))) ?></td>
                            <td>
                                <span class="<?= emploi_badge_class($emploi['statut']) ?>">
                                    <?= htmlspecialchars($emploi['statut'] === 'termine' ? theme_t('Termine', 'منتهية') : ($emploi['statut'] === 'annule' ? theme_t('Annule', 'ملغاة') : theme_t('Planifie', 'مخطط'))) ?>
                                </span>
                            </td>
                            <td>
                                <div class="actions">
                                    ...
                                    <a href="<?= htmlspecialchars(theme_url('VIEW/backoffice/admin-emplois-supprimer.php?id=' . $emploi['id_emploi'])) ?>" class="btn btn--danger" onclick="return confirm('<?= htmlspecialchars(theme_t('Supprimer cet emploi ?', 'حذف هذا الجدول؟')) ?>');">
    ...
    <?php else: ?>
        <div class="empty-state">
            <h3><?= htmlspecialchars(theme_t('Aucun emploi trouve', 'لا توجد جداول')) ?></h3>
            <p><?= htmlspecialchars(theme_t('Ajoutez un planning pour commencer a organiser les affectations.', 'اضف جدولا لبدء تنظيم التعيينات.')) ?></p>
        </div>
    <?php endif; ?>
</div>
