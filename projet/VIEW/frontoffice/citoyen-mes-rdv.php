...
$list = $rdvController->getRendezVousByCitoyen($citoyen_nom);
?>
                ...
                <tbody>
                    <?php if(!empty($list)): ?>
                        <?php foreach($list as $rdv): ?>
                            <?php 
                            // Un rendez-vous passé ou annulé ne peut pas être modifié
                            $date_rdv = strtotime($rdv['date_heure']);
                            $date_now = time();
                            $estPasse = ($date_rdv < $date_now);
                            $disabled = ($estPasse || $rdv['statut'] == 'termine') ? 'disabled' : '';
                            ?>
                            <tr>
                                <td>#<?= $rdv['id_rdv'] ?></td>
                                ...
                                <td><?= $rdv['agent_nom'] ?? '<span class="badge badge-en_attente">Non affecté</span>' ?></td>
                                <td><?= htmlspecialchars($rdv['motif'] ?? '-') ?></td>
                                <td>
                                    <a href="/projet/VIEW/frontoffice/citoyen-modifier-rdv.php?id=<?= $rdv['id_rdv'] ?>" 
                                       class="btn btn-warning btn-sm <?= $disabled ?>" <?= $disabled ?>>
                                        <i class="fas fa-edit"></i> Modifier
                                    </a>
                                    <a href="/projet/VIEW/frontoffice/citoyen-annuler-rdv.php?id=<?= $rdv['id_rdv'] ?>" 
                                       class="btn btn-danger btn-sm" 
                                       onclick="return confirm('⚠️ Attention : Cette action va SUPPRIMER définitivement ce rendez-vous. Êtes-vous sûr ?')">
