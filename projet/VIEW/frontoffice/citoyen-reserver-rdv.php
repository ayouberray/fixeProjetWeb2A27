...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($id_service) && !empty($date_heure)){
        ...
        $stmt = $db->prepare("SELECT nom_service FROM services WHERE id_service = :id");
        ...
        $rdv = new RendezVous($citoyen_nom, $service_nom, $date_heure, $motif);
        $result = $rdvController->ajouterRendezVous($rdv);
        if($result){ 
            $success = "Rendez-vous réservé avec succès ! ID: #".$result; 
        } else { 
            ...
            <div class="form-group">
                ...
                <select name="id_service" class="form-control" required>
                    ...
                    <?php foreach($services as $s): ?>
                        <option value="<?= $s['id_service'] ?>"><?= htmlspecialchars($s['nom_service']) ?> (<?= $s['duree_moyenne'] ?> min)</option>
                    <?php endforeach; ?>
                </select>
            </div>
